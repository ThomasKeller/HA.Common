...
public class MqttPublisher : IMqttPublisher, IObserverProcessor
{
    ...
    public MqttPublisher(ILogger<MqttPublisher> logger, MqttClientOptions clientOptions, string? clientId = null)
    {
        ...
        Task.Factory.StartNew(() => EnsureConnected());
    }
    ...
    {
        var isConnected = await EnsureConnected();
        if (isConnected)
        {
    ...
    public async Task<bool> Publish(IDictionary<string, string> topicsAndpPayloads)
    {
        var isConnected = await EnsureConnected();
        if (isConnected)
        {
    ...
    public async Task<bool> Publish(Measurement measurement)
    {
        var isConnected = await EnsureConnected();
        if (isConnected)
        {
    ...
    {
        AsyncHelper.RunSync(() => Publish(measurement));
    }
    ...
    private async Task<bool> PublishMessage(MqttApplicationMessage message)
    {
